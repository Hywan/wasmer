...
impl Instance {
    pub(crate) fn new(module: Arc<ModuleInner>, imports: &ImportObject) -> Result<Instance> {
        ...
        if let Some(start_index) = instance.module.info.start_func {
            ...
            let start_func: Func<(), (), Wasm> = unsafe {
                Func::from_raw_parts(
                    wasm_trampoline,
                    func_ptr,
                    func_env_ptr,
...
fn call_func_with_index(
    ...
    call_func_with_index_inner(vmctx, func_env_ptr, func_ptr, signature, wasm, args, rets)
}

pub(crate) fn call_func_with_index_inner(
    vmctx: Option<NonNull<vm::Ctx>>,
    func_env_ptr: Option<NonNull<vm::FuncEnv>>,
...
impl<'a> DynFunc<'a> {
    ...
    pub fn call(&self, params: &[Value]) -> CallResult<Vec<Value>> {
        ...
        call_func_with_index(
            ...
            NonNull::new(self.instance_inner.vmctx),
            self.func_index,
            params,
            &mut results,
        )?;
